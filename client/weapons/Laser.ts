...
export default class Laser implements IWeapon {
  ...
  fire (from: IGameUnit, to: IGameUnit, scene: BABYLON.Scene): boolean {
    ...
    let distance = Formations.Distance2D(from.mesh.position, to.mesh.position)

    let mesh: BABYLON.Mesh = BABYLON.Mesh.CreateCylinder('cylinder', distance, 0.02, 0.2, 36, 2, scene, true)

    mesh.setPivotMatrix(BABYLON.Matrix.Translation(0, -distance / 2, 0))
    mesh.position = from.mesh.position

    let v1 = from.mesh.position.subtract(to.mesh.position)
    v1.normalize()
    ...
    let material: StandardMaterial = new BABYLON.StandardMaterial('green', scene)
    ...
    let animationFadeOut = new BABYLON.Animation('animationCore', 'position', common.ANIMATIONS_FPS, BABYLON.Animation.ANIMATIONTYPE_VECTOR3, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT)
    /*    let keys = []
